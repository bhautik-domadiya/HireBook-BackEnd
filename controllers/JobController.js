...
exports.JobList = [
  ...
  function (req, res) {
    try {
      ...
      UserModel.findOne({ _id: mongoose.Types.ObjectId(req.user.id) })
        .populate({
          path: "jobseeker",
          model: "JobSeeker",
        })
        .exec((err, userData) => {
          ...
          // Filter Jobs by not interested if Jobseeker is fecthing jobs
          if (
            userData &&
            userData.jobseeker &&
            userData.jobseeker.notInterestedJobs
          ) {
            filterObj._id = { $nin: userData.jobseeker.notInterestedJobs };
          }
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        JobModel.findOneAndUpdate(
          { _id: mongoose.Types.ObjectId(req.body.jobId), postedBy: user.id },
          { $set: { isDeleted: true } },
          (err, job) => {
            if (err) {
              return apiResponse.validationErrorWithData(
